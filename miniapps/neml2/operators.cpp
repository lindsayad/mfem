...
LinearMomentumBalance<dscalar_t>::LinearMomentumBalance(ParFiniteElementSpace &fe_space,
                                                        const IntegrationRule &ir,
                                                        std::shared_ptr<neml2::Model> cmodel,
                                                        DerivativeType deriv_type)
      ...
      _strain(q_space_symr2), _stress(q_space_symr2)
{
   ...
   const auto strain_derivatives = std::integer_sequence<std::size_t,
                                                         SOLUTION>{};
   strain_op->AddDomainIntegrator(strain_qfunc,
                                  tuple{Gradient<SOLUTION>{},
                                        Gradient<NODAL_COORDS>{}},
   ...
   stressdiv_op = std::make_unique<DifferentiableOperator>(fields, params,
                                                           pmesh);
   StressDivergence balance_qfunc;
   const auto residual_derivatives = std::integer_sequence<std::size_t,
                                                           STRESS>{};
   stressdiv_op->AddDomainIntegrator(balance_qfunc,
                                     tuple{Identity<STRESS>{},
                                           Gradient<NODAL_COORDS>{}, Weight{}},
                                     tuple{Gradient<SOLUTION>{}}, ir,
                                     everywhere, residual_derivatives);
}
...
Operator &LinearMomentumBalance<dscalar_t>::GetGradient(const Vector &X) const
{
   ...
   {
      case HANDCODED:
         _man_deriv_op = std::make_unique<LinearMomentumBalanceJacobian>(*this,
                                                                         X);
         return *_man_deriv_op;
      case AUTODIFF:
      default:
         _autodiff_deriv_op = std::make_unique<AutodiffLinearMomentumBalanceJacobian>(this,
                                                                                      X);
...
template <typename dscalar_t>
LinearMomentumBalance<dscalar_t>::LinearMomentumBalanceJacobian::LinearMomentumBalanceJacobian(
                                                                                const LinearMomentumBalance &op,
                                                                                const Vector &X)
...
template <typename dscalar_t>
void LinearMomentumBalance<dscalar_t>::LinearMomentumBalanceJacobian::Mult(const Vector &dX,
                                                                           Vector &dR)
                                                                                const
{
   // essential boundary conditions
...
template <typename dscalar_t>
LinearMomentumBalance<dscalar_t>::AutodiffLinearMomentumBalanceJacobian::AutodiffLinearMomentumBalanceJacobian(
                                                                                const LinearMomentumBalance *momentum_balance,
                                                                                const Vector &x)
      ...
      _delta_strain(_q_space_symr2), _delta_stress(_q_space_symr2)
{
   ...
   // Evaluate the tangent at the current state
   momentum_balance->strain_op->SetParameters({&momentum_balance->nodes,
                                               &_strain});
   ...
   dstrain_du = momentum_balance->strain_op->GetDerivative(LinearMomentumBalance::SOLUTION,
                                                           {&momentum_balance->u},
                                                           {&momentum_balance->nodes,
                                                            &_strain});
   dres_dstress = momentum_balance->stressdiv_op->GetDerivative(LinearMomentumBalance::STRESS,
                                                                {&momentum_balance->u},
                                                                {&momentum_balance->nodes,
                                                                 &_stress});
}
...
template <typename dscalar_t>
void LinearMomentumBalance<dscalar_t>::AutodiffLinearMomentumBalanceJacobian::Mult(
                                                                                const Vector &dX,
                                                                                Vector &dR)
                                                                                const
{
